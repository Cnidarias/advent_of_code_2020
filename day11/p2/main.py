...
def get_occupied_seats(row, col, layout_in):
    ...
    for row_incr, col_incr in directions:
        current_row = row + row_incr
        current_col = col + col_incr
        while 0 <= current_row < len(layout_in) and 0 <= current_col < len(layout_in[0]):
            if layout_in[current_row][current_col] == "L":
                break
            if layout_in[current_row][current_col] == "#":
                count += 1
                break
            current_row += row_incr
            current_col += col_incr
    ...
    for row, row_element in enumerate(layout_in):
        ...
        for col, col_element in enumerate(row_element):
            ...
            if col_element == 'L' and surrounding_occupied_seats == 0:
                new_place = "#"
            if col_element == '#' and surrounding_occupied_seats >= 5:
                new_place = 'L'
            new_row += new_place
